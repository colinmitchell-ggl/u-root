...
const (
	...
	// This arbitrarily value is defined in testdata/README.md for data creation.
	imageSize = 100
...
func compareImage(t *testing.T, name string, wantFill string, image io.ReaderAt) {
	...
	// the imageSize+1 byte. Therefore compare with gotImage[0:imageSize].
	...
	if gotSize != imageSize {
		t.Errorf("failed reading %s passed to loadImage: bytes got %v, want %v", name, gotSize, imageSize)
	}
	wantInit := fillPattern(wantFill, imageSize)
	if !bytes.Equal(gotImage[0:imageSize], wantInit) {
		t.Errorf("loadImage gave %s: %v, want pattern: \"%v...\"", name, gotImage, wantFill)
	}
}
	...
	for _, tt := range []struct {
		...
		t.Run(tt.desc, func(t *testing.T) {
			...
			if b != nil {
				...
				if err != nil {
					t.Fatalf("Could not read content: %v", err)
				}
				wantContent := fillPattern(tt.wantContentFill, imageSize)
				if !bytes.Equal(content, wantContent) {
					t.Errorf("ReadAll = %v, want pattern: \"%v...\"", content, tt.wantContentFill)
				}
			}
		})
	}
...
// fillPattern doubles a pattern until meeting size
func fillPattern(pattern string, size int) []byte {
	if pattern == "" {
		return nil
	}
	var filled = make([]byte, size, size)
	copy(filled, pattern)
	for i := len(pattern); i < len(filled); i *= 2 {
		copy(filled[i:], filled[:i])
	}
	return filled
}
