...
//
// vfit aims to be TOCTTOU-safe by reading files into memory before verifying.
package fit
...
// parseHash cleans and maps a string into a crytpo.Hash.
...
func parseHash(algo string) (crypto.Hash, error) {
	...
	if alg, ok := algs[strings.ToUpper(algoSplit[0])]; ok {
		return alg, nil
	}
	return 0, fmt.Errorf("Unrecognized hash algo: '%s'", cleaned)
}
